plot.split.s.am <- function(sp.name,method.name,color.choice="venette",save.fig=F,exp=F,xlim=NULL,ylim=NULL,pdf=F) {
...
# azure2		224, 238, 238
	...
	if (is.null(xlim)) win.graph(width=55,height=70)
	if (!is.null(xlim)) { 
#		win.graph() 
		cex1 <- 0.8
		pch1 <- 15
