...
#   _____                                  _     _           _            _                             
#  / ____|                                | |   (_)         | |     /\   | |                            
# | |  __  ___  ___   __ _ _ __ __ _ _ __ | |__  _  ___ __ _| |    /  \  | |__  ___  ___ _ __   ___ ___ 
# | | |_ |/ _ \/ _ \ / _` | '__/ _` | '_ \| '_ \| |/ __/ _` | |   / /\ \ | '_ \/ __|/ _ | '_ \ / __/ _ \
# | |__| |  __| (_) | (_| | | | (_| | |_) | | | | | (_| (_| | |  / ____ \| |_) \__ |  __| | | | (_|  __/
#  \_____|\___|\___/ \__, |_|  \__,_| .__/|_| |_|_|\___\__,_|_| /_/    \_|_.__/|___/\___|_| |_|\___\___|
#                     __/ |         | |                                                                 
...
#    | | | | | '_ \ / _ \ |______|   | |   | |   | |                                                    
#    | | |_| | |_) |  __/           _| |_ _| |_ _| |_                                                   
#    |_|\__, | .__/ \___|          |_____|_____|_____|                                                  
#        __/ | |                                                                                        
#       |___/|_|                                                                                       

# Author: Tak Ikeda, all paths are edited to suit Senait's preference 
#Changes from the original script this script uses externally provided geog bounded random absence points, it doesn't run the OCSVM and K-means clustering to get an environmentally
...
species.name <- "aa31"  # Aedes albopicta method 3 replicate 1
...
#
setwd('C:\\MMA\\db\\PAG')
#1 add a column pres on both the presence and random absence points, for presence use species.name_worldclim.txt file in _dist_data folder and absenc the external geog. bounded random points
...
transaa31_1= read.csv('C:\\MMA\\db\\PAG\\aa31_data_pres_abs_threshold_0_prev_50.csv', header =T)
transaa31=as.data.frame(transaa31_1)
setwd(paste("C:\\MMA\\",species.name,"\\_dist_data_OCSVM",sep=''))
write.table(transaa31,file=paste(species.name,"_data_pres_abs_threshold_0_prev_50.txt",sep=""),sep = " ",row.names=F)
...
setwd(paste("C:\\MMA\\",species.name,"\\_dist_data_OCSVM",sep=""))
#import data
aa31data=read.table(paste(species.name,"_data_pres_abs_best.txt",sep=""),header=T) #import 2*2933 abs pres data from aa31 mma output
#creating random 366 presence points out of 2933 presence data for parametrizing AA31
pres=aa31data[(aa31data[,8]==1),] #separate presence points,check number of columns
...
abse=aa31data[(aa31data[,8]==0),] #separate absence points
...
aa_data_pres_abs_best_3_1=rbind(pres,abse)
aa_data_pres_abs_best_3<- as.data.frame(aa_data_pres_abs_best_3_1)
...
# ------------ #
	...
	setwd(paste("C:\\MMA\\",species.name,"\\_dist_data_OCSVM",sep=""))
	data <- read.table(paste(species.name,"_data_pres_abs_best_3.txt",sep=""), header=TRUE)
...
#--------------------------------------------------------#
#save workspace
save.image(paste("C:\\MMA\\",species.name,"\\aa31_world.RData",sep=""))
#save history
savehistory(file = (paste("C:\\MMA\\",species.name,"\\aa31_world.Rhistory",sep="")))
...
#######################################################################################################################################################################
#save workspace
save.image(paste("C:\\MMA\\",species.name,"\\aa_3_1.RData",sep=""))
#save history
savehistory(file = (paste("C:\\MMA\\",species.name,"\\aa_3_1.Rhistory",sep="")))
